...
async function loadHotspots(modelFile) {
  ...
  hotspots.forEach(h => {
    ...
    wrapper.querySelector("button").addEventListener("click", () => {
      ...
      document.getElementById("popup-description").textContent = h.info.description;
...
function animateCamera() {
  ...
  modelViewer.cameraOrbit = `${theta.toFixed(2)}deg ${phi.toFixed(2)}deg ${radius.toFixed(2)}m`;
  modelViewer.jumpCameraToGoal();
